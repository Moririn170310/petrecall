...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['dog_image']) && $_FILES['dog_image']['error'] === UPLOAD_ERR_OK) {
        $imageData = file_get_contents($_FILES['dog_image']['tmp_name']);

        // データベースに保存
        $stmt = $pdo->prepare("INSERT INTO pets (dog_name, user_name, email, password, dog_image, address, phone) 
                               VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$dogName, $userName, $email, $password, $imageData, $address, $phone]);
    ...
    <div class="bg-white p-8 rounded-lg shadow-md w-96">
        ...
        <form action="regist.php" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <textarea name="address" class="mt-1 block w-full p-2 border rounded-md" required></textarea>
            </div>
            <div class="mb-4">
                <label class="block text-gray-700">電話番号</label>
                <input type="text" name="phone" class="mt-1 block w-full p-2 border rounded-md" required>
            </div>
            <div class="mb-4">
                <label class="block text-gray-700">犬の画像</label>
                <input type="file" name="dog_image" class="mt-1 block w-full p-2 border rounded-md" required>
            </div>
            <button type="submit" class="bg-blue-500 text-white p-2 rounded-md w-full">登録</button>
        </form>
    </div>
</body>
</html>
